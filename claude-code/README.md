...
- Claude Codeが自動生成するトピックタイトル（surfaceタイトル）を読み取り、`claude -p --model haiku` で**日本語11文字以内**のタブ名に変換して `rename-workspace` する（例: `2:Claude PP週次レポート`）
- 先頭の `2:` は **⌘+数字で飛ぶときの番号**（サイドバーの並び順 = `list-workspaces` の `index`+1）。hookが走るたびに**全タブぶん振り直す**ので、タブを追加・削除・並べ替えしても番号がズレない。番号を付けるのはこのhookが管理するタブ（`Claude ` / `Codex ` / 旧 `✳` / `◌` 始まり）だけで、素のターミナルタブはリネームしない（リネームするとOSCタイトル追従が止まるため）
- 同一トピックは `$TMPDIR/cmux_tab_title_<WORKSPACE_ID>` にキャッシュし、API再呼び出しなし
- `/clear` や新規起動時は `Claude 待機` 表示にリセット（`--blank` モード、SessionStart hook）。resume/compactでは既存タブ名を維持
...
- エージェント名の表示幅を確保するため、日本語要約は9文字以内
